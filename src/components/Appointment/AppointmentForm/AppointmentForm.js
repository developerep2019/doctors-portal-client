...
const AppointmentForm = ({ closeModal, isModalOpen, afterOpenModal, appointmentOn, date }) => {
    const customStyles = {
        content: {
            top: '50%',
            left: '50%',
    ...
    const onSubmit = data => {
        ...
        fetch('http://localhost:5000/addAppointment', {
            ...
            .then(success => {
                if (success) {
                    closeModal();
                    alert('Thanks for your Appointment');
                }
            })
            ...
            <Modal isOpen={isModalOpen} onAfterOpen={afterOpenModal} onRequestClose={closeModal} style={customStyles} contentLabel="Example Modal">
                ...
                <form className="p-5" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="form-group text-right">
                        <button type="submit" className="btn btn-brand text-uppercase">book appointment</button>
                    </div>
                </form>
            </Modal>
        </div>
